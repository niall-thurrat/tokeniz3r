...
- **Tokenizer**: this is the class that is exported through the index.js file. It's public methods and (pseudo) properties constitute the module's public interface.
...
- **Token**: instances are created by the app to represent best matches and can be got by users using the activeToken property, or set using the setActiveTokenToNext and setActiveTokenToPrevious methods on the Tokenizer class.
...
- For this lab I have written code that is much wider than I normally write. There are 2 main factors for this: tying to stick to the **small** rule (i.e. reduce line count) and trying to use **intention revealing names** that mean I have probably added more info in the names than I normally would.
