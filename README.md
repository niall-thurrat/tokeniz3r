...
**getBestMatchingToken(strToMatch, isForPrevious)** - This method might be useful to test how the grammar in a Tokenizer instance handles various strings, both for next and previous token matches.*** The *isForPrevious* argument is a boolean used to switch between next or previous token rules in the app. If (isForPrevious === true) then you will get the 'best matching token' that is found at the end of your string.
